-- amp_invoices 
CREATE TABLE IF NOT EXISTS amp_invoices (
    ...
    -- If settled, the index is set to the current_value+1 of the settled_index
    -- seuqence in the invoice_sequences table. If not settled,then it is set
    -- to NULL.
    settled_index BIGINT,
...
CREATE INDEX IF NOT EXISTS amp_invoices_invoice_id_idx ON amp_invoices(invoice_id);
...
CREATE TABLE IF NOT EXISTS amp_invoice_htlcs (
    ...
    set_id BLOB NOT NULL REFERENCES amp_invoices(set_id) ON DELETE CASCADE,
